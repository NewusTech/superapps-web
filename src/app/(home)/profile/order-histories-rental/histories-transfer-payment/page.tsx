...
export default function HistoriesTransferPaymentPage() {
  ...
  useEffect(() => {
    if (
      ...
      setCodePayment(localStorage.getItem("payment_code") || "");
  ...
  const handleRouterHistory = () => {
    router.push(
      `/profile/order-histories-rental/histories-rental-detail/${codePayment}`
    );
    localStorage.clear();
  };

  return (
    <section className="flex flex-col w-full md:w-full h-full justify-center items-center relative md:mb-0 pb-36 md:pb-80">
      <div className="w-full flex flex-col mt-5 md:mt-32 gap-y-5">
        <div className="w-full rounded-lg flex px-5 flex-col border border-grey-100 shadow-md gap-y-3">
          <div className="w-full flex flex-col py-2">
            <p className="font-bold text-neutral-700 text-[18px] md:text-[22px]">
